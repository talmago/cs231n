...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # and biases. Store the results in the grads dictionary. For example, grads['W1']
        # should store the gradient on W1, and be a matrix of same size.
        grads = {}

        # Compute the delta in scores
        delta = probs
        delta[np.arange(N), y] -= 1
        delta /= N

        # backpropagate into W2 and b2
        grads['W2'] = np.dot(X1.T, delta) + reg * W2
        grads['b2'] = np.sum(delta, axis=0, keepdims=True)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            # Create a random minibatch of training data and labels, storing them
            # in X_batch and y_batch respectively.
            ...
            # Use the gradients in the grads dictionary to update the parameters of the
            # network (stored in the dictionary self.params) using stochastic
            # gradient descent. You'll need to use the gradients stored in the grads
            # dictionary defined above.
            for param in self.params:
                grads[param].shape = self.params[param].shape
    ...
    def predict(self, X):
        ...
        # probabilities
        X2_exp = np.exp(X2)
        probs = X2_exp / np.sum(X2_exp, axis=1, keepdims=True)
